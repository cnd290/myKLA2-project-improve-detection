...
(function(global, factory) {
    ...
    ***************************************************************************** */
    ...
    ENV.registerFlag('WEBGL_DISJOINT_QUERY_TIMER_EXTENSION_RELIABLE', function() {
        return ENV.getNumber('WEBGL_DISJOINT_QUERY_TIMER_EXTENSION_VERSION') > 0 &&
    ...
    function getSamplerScalar(inputInfo) {
        ...
        var _a = inputInfo.shapeInfo.texShape,
            texNumR = _a[0],
            texNumC = _a[1];
        if (texNumR === 1 && texNumC === 1) {
            return "\n      float " + funcName + "() {\n        return sampleTexture(" + texName + ", halfCR);\n      }\n    ";
        }
        var _b = inputInfo.shapeInfo.texShape,
            tNumR = _b[0],
            tNumC = _b[1];
    ...
    function getSampler2D(inputInfo) {
        ...
        var _a = tf.util.squeezeShape(shape),
    ...
    var ArgMinMaxPackedProgram = /** @class */ (function() {
        function ArgMinMaxPackedProgram(shape, windowSize, op, firstPass) {
            ...
            tf.util.assert(shape.length > 2, function() {
                return "Packed arg" + (op.charAt(0).toUpperCase() +
    ...
    var CropAndResizeProgram = /** @class */ (function() {
        function CropAndResizeProgram(imageShape, boxShape, cropSize, method, extrapolationValue) {
            ...
            var _a = [imageHeight - 1 + ".0", imageWidth - 1 + ".0"],
                inputHeightFloat = _a[0],
                inputWidthFloat = _a[1];
            var _b = cropHeight > 1 ? [
    ...
    function createFloat32MatrixTexture(gl, rows, columns, textureConfig) {
        var _a = getUnpackedMatrixTextureShapeWidthHeight(rows, columns),
            width = _a[0],
            height = _a[1];
        ...
        GPGPUContext.prototype.waitForQueryAndGetTime = function(query) {
            return __awaiter(this, void 0, void 0, function() {
                ...
                return __generator(this, function(_a) {
                    switch (_a.label) {
                        case 0:
                            return [4 /*yield*/ , tf.util.repeatedTry(function() {
    ...
    function computeBytes(shape, physicalTexType, gl, textureConfig, isPacked) {
        ...
        var numElements;
        if (isPacked) {
            var _a = getPackedMatrixTextureShapeWidthHeight(shape[0], shape[1]),
                packedWidth = _a[0],
                packedHeight = _a[1];
            numElements = packedWidth * packedHeight;
        } else {
            var _b = getUnpackedMatrixTextureShapeWidthHeight(shape[0], shape[1]),
                width = _b[0],
                height = _b[1];
    ...
    var MathBackendWebGL = /** @class */ (function(_super) {
        ...
        MathBackendWebGL.prototype.getValuesFromTexture = function(dataId) {
            var _a;
            var _b = this.texData.get(dataId),
                shape = _b.shape,
                dtype = _b.dtype,
                isPacked = _b.isPacked;
            var size = tf.util.sizeFromShape(shape);
            if (tf.env().getBool('WEBGL_DOWNLOAD_FLOAT_ENABLED')) {
        ...
        MathBackendWebGL.prototype.shouldExecuteOnCPU = function(inputs, sizeThreshold) {
            ...
            return cpuBackend != null &&
                inputs.every(function(input) {
                    return _this.texData.get(input.dataId).texture == null &&
        ...
        MathBackendWebGL.prototype.batchToSpaceND = function(x, blockShape, crops) {
            tf.util.assert(x.rank <= 4, function() {
                return 'batchToSpaceND for rank > 4 with a WebGL backend not ' +
                    'implemented yet';
            });
        ...
        MathBackendWebGL.prototype.spaceToBatchND = function(x, blockShape, paddings) {
            tf.util.assert(x.rank <= 4, function() {
                return 'spaceToBatchND for rank > 4 with a WebGL backend not ' +
                    'implemented yet';
            });
            var prod = blockShape.reduce(function(a, b) { return a * b; });
        ...
        MathBackendWebGL.prototype.sum = function(x, axes) {
            ...
            var _a = tf.backend_util.computeOutAndReduceShapes(x.shape, axes),
                outShape = _a[0],
                reduceShape = _a[1];
        ...
        MathBackendWebGL.prototype.scatterND = function(indices, updates, shape) {
            var _a = tf.backend_util.calculateShapes(updates, indices, shape),
        ...
        MathBackendWebGL.prototype.sparseToDense = function(sparseIndices, sparseValues, outputShape, defaultValue) {
            var _a = tf.backend_util.calculateShapes(sparseValues, sparseIndices, outputShape),
                sliceRank = _a.sliceRank,
                numUpdates = _a.numUpdates,
                strides = _a.strides,
                outputSize = _a.outputSize;
        ...
        MathBackendWebGL.prototype.gatherND = function(x, indices) {
            ...
            var _a = tf.backend_util.prepareAndValidate(x, indices),
                resultShape = _a[0],
                numSlices = _a[1],
                sliceSize = _a[2],
                strides = _a[3];
    ...
    function reduce(x, dtype, reductionType, backend) {
        ...
        for (var i = 0; i < reductionStages.length; i++) {
            var _a = reductionStages[i],
                inSize = _a.inSize,
                windowSize = _a.windowSize,
                outSize = _a.outSize;
    ...
    var maxConfig = {
        ...
        kernelFunc: function(_a) {
            ...
            var x = inputs.x;
            var _b = attrs,
                reductionIndices = _b.reductionIndices,
            ...
            var _c = tf.backend_util.computeOutAndReduceShapes(maxInput.shape, axes),
                maxOutShape = _c[0],
                reduceShape = _c[1];
    ...
    var maxPoolWithArgmaxConfig = {
        ...
        kernelFunc: function(_a) {
            ...
            var _b = attrs,
                ...
                strides = _b.strides,
                pad = _b.pad,
                includeBatchInIndex = _b.includeBatchInIndex;
            var webglBackend = backend;
            ...
            var _c = maxPoolWithArgmaxImpl(x, includeBatchInIndex, convInfo, webglBackend),
                result = _c[0],
                indexes = _c[1];
    ...
    var nonMaxSuppressionV4Config = {
        ...
        kernelFunc: function(_a) {
            ...
            var boxesVals = gpuBackend.readSync(boxes.dataId);
            var scoresVals = gpuBackend.readSync(scores.dataId);
            var _d = nonMaxSuppressionV4Impl(boxesVals, scoresVals, maxOutputSize, iouThreshold, scoreThreshold, padToMaxOutputSize),
                selectedIndices = _d.selectedIndices,
                validOutputs = _d.validOutputs;
    ...
    var nonMaxSuppressionV5Config = {
        ...
        kernelFunc: function(_a) {
            ...
            var _d = nonMaxSuppressionV5Impl(boxesVals, scoresVals, maxOutputSizeVal, iouThresholdVal, scoreThresholdVal, softNmsSigmaVal),
                selectedIndices = _d.selectedIndices,
                selectedScores = _d.selectedScores;
    ...
    var RotateProgram = /** @class */ (function() {
        function RotateProgram(imageShape, radians, fillValue, center) {
            ...
            this.outputShape = imageShape;
            var _a = tf.backend_util.getImageCenter(center, imageHeight, imageWidth),
                centerX = _a[0],
                centerY = _a[1];
